...
// export default function SelectFreeGift({
//   groupId,
//   activeTabIndex, // received from parent Groups component
//   associatedMetafields,
// }) {
//   const app = useAppBridge();
//   const [products, setProducts] = useState([]);
...
//   // Instead of a single array, we store selections per group (keyed by activeTabIndex)
//   const [selectedProductsByGroup, setSelectedProductsByGroup] = useState({});
//   const [initialProductsByGroup, setInitialProductsByGroup] = useState({});
...
//       // Extract gift variants from metafields
//       const giftVariantsMetafield = product.metafields?.edges?.find(
//         (metafield) => metafield.node.key === "recommendations",
//       );
...
//     // Build new text input values and selected products array.
//     const newTextInputValues = {};
//     const newSelectedProductsArray = [];
...
//     console.log("formattedProducts", formattedProducts);
//     console.log("newSelectedProductsArray", newSelectedProductsArray);
...
//     // Save selections for the current group (activeTabIndex)
//     setSelectedProductsByGroup((prev) => ({
//       ...prev,
//       [activeTabIndex]: append
//         ? [...(prev[activeTabIndex] || []), ...newSelectedProductsArray]
//         : newSelectedProductsArray,
//     }));
//     setInitialProductsByGroup((prev) => ({
//       ...prev,
//       [activeTabIndex]: append
...
//   // Reload products when search term or active group changes.
//   useEffect(() => {
//     loadProducts();
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [searchTerm, activeTabIndex]);
...
//     if (associatedMetafields && associatedMetafields.length > 0) {
//       setSelected(associatedMetafields[0].key);
//       const selectedMetafield = associatedMetafields[0];
//       const isProductType =
//         selectedMetafield?.type?.name === "list.product_reference" ||
//         selectedMetafield?.type?.name === "product_reference";
//       setIsProductMetafield(isProductType);
...
//     const groupSelected = selectedProductsByGroup[activeTabIndex] || [];
//     const groupInitial = initialProductsByGroup[activeTabIndex] || [];

//     // Loop through the products in this group and update changed selections.
//     for (const entry of groupSelected) {
//       const productId = entry.productId;
//       const initialEntry = groupInitial.find((item) => item.productId === productId);
//       const currentSelections = entry.selections;
...
//     ) {
//       setSelectedProductsByGroup((prev) => {
//         const groupSelections = prev[activeTabIndex] || [];
//         const idx = groupSelections.findIndex((entry) => entry.productId === productId);
//         let existingSelections = [];
//         if (idx !== -1) {
//           existingSelections = groupSelections[idx].selections;
//         }
//         const newSelections = pickerResult.selection.map((item) => ({
//           id: item.id,
//           title: item.title,
...
//         setHasChanges(true);
//         if (idx !== -1) {
//           const updatedGroupSelections = groupSelections.map((entry) =>
//             entry.productId === productId
//               ? { productId, selections: uniqueSelections }
//               : entry
//           );
//           return { ...prev, [activeTabIndex]: updatedGroupSelections };
//         } else {
//           return {
//             ...prev,
//             [activeTabIndex]: [
...
//     const idx = groupSelections.findIndex((entry) => entry.productId === productId);
//     if (idx !== -1) {
//       const updatedSelections = groupSelections[idx].selections.filter(
//         (item) => item.id !== selectedItemId,
//       );
//       setHasChanges(true);
//       const updatedGroupSelections = groupSelections.map((entry) =>
//         entry.productId === productId
//           ? { productId, selections: updatedSelections }
//           : entry
//       );
//       return { ...prev, [activeTabIndex]: updatedGroupSelections };
//     }
...
//                       console.log("rendering item", item);
//                       const { id, title } = item;
//                       // Get selections for current group using activeTabIndex.
//                       const groupSelections = selectedProductsByGroup[activeTabIndex] || [];
//                       const entry = groupSelections.find((e) => e.productId === id);
//                       const selectedItems = entry ? entry.selections : [];
//                       const metafieldKey = selected;
//                       const selectedMetafield = options.find(
//                         (option) => option.value === selected,
...
//                               <Button
//                                 onClick={() =>
//                                   openResourcePicker(id, selectedMetafield?.type?.name)
//                                 }
//                               >
//                                 Select Product/Variant
//                               </Button>
